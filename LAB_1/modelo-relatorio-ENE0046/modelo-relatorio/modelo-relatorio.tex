...
Os esquemáticos deste documento foram redesenhados em Circuitikz a partir dos arquivos
\texttt{item\_1.asc} a \texttt{item\_6.asc}; os gráficos foram reconstruídos numericamente com os
mesmos modelos ideais e receberam cores e fundo próximos aos do visualizador do LTspice.
...
Os itens 1 e 2 usam o divisor da Figura~\ref{fig:divisor}. A diretiva
...
\node[align=left] at (2.5,-0.8) {\texttt{R1=1.2k; R2=2.2k}\\\texttt{.step param RL list 1000 1000meg}};
\end{circuitikz}
\caption{Divisor resistivo dos itens 1 e 2, com a carga parametrizada.}
\label{fig:divisor}
\end{figure}
...
\centering
\begin{circuitikz}[scale=1.05]
\draw (0,0) node[ground]{} to[V,l_={$V_1\;(AC\;1)$}] (0,2.8)
  to[R,l_={$R_1=3{,}9\,\mathrm{k}\Omega$}] (3.4,2.8) coordinate(out)
  to[C,l_={$C_1=100\,\mathrm{nF}$}] (3.4,0) -- (0,0);
\draw (out) node[circ]{} node[above]{$v_{out}$};
\node at (1.7,-0.7) {\texttt{.ac dec 100 3.88 38800}};
\end{circuitikz}
\caption{Filtro RC passa-baixas do item 3.}
\label{fig:rc-ac}
\end{figure}

Nos itens 4 e 5, o RLC da Figura~\ref{fig:rlc} usa
...
\draw (0,0) node[ground]{} to[V,l_={$V_1$}] (0,2.8)
  to[R,l_={$R_1=\{R\}$}] (2.4,2.8)
  to[L,l_={$L_1=100\,\mathrm{mH}$}] (5.0,2.8) coordinate(out)
  to[C,l_={$C_1=1\,\mathrm{nF}$}] (5.0,0) -- (0,0);
\draw (out) node[circ]{} node[above]{$v_{out}$};
\node at (2.5,-0.7) {\texttt{.step param R list 4700 12000 22000}};
\end{circuitikz}
\caption{RLC série dos itens 4 e 5, com saída sobre o capacitor.}
\label{fig:rlc}
\end{figure}
...
\draw (0,0) node[ground]{} to[sV,l_={$V_1$}] (0,2.8)
  ...
  to[C,l_={$C_1=100\,\mathrm{nF}$}] (3.4,0) -- (0,0);
\draw (out) node[circ]{} node[above]{$v_{out}$};
\node[align=center] at (1.7,-0.75) {\texttt{.tran 0 51.55m 12.89m 8.591u}\\\texttt{.options plotwinsize=0}};
\end{circuitikz}
\caption{Filtro RC excitado por onda quadrada no item 6.}
\label{fig:rc-pulse}
\end{figure}
...
No ponto de operação, o divisor fornece os valores da Tabela~\ref{tab:op}. A varredura da
...
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-2-dc.png}
\caption{Varredura DC reconstruída com aparência próxima ao visualizador do LTspice.}
\label{fig:dc}
\end{figure}
...
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-3-bode.png}
\caption{Magnitude e fase do filtro RC, com referência e corte efetivo destacados.}
\label{fig:bode-rc}
\end{figure}
...
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-4-transiente.png}
\caption{Resposta ao degrau do RLC para os três valores de resistência.}
\label{fig:rlc-tran}
\end{figure}

\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-5-bode.png}
\caption{Resposta em frequência do RLC para os três valores de resistência.}
\label{fig:rlc-bode}
\end{figure}
...
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-6-tempo.png}
\caption{Entrada quadrada e saída do filtro RC em regime permanente.}
\label{fig:pulse}
\end{figure}
...
\begin{figure}[htb]
\centering
\includegraphics[width=.9\linewidth]{figuras/grafico-item-6-espectro.png}
\caption{Espectro da entrada quadrada e das componentes após o filtro RC.}
\label{fig:fft}
\end{figure}
